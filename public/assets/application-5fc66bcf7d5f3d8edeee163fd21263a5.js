			...
			if ( match && (match[1] || !context) ) {
				...
				// HANDLE: $(#id)
					...
					if ( elem && elem.parentNode ) {
						// Handle the case where IE and Opera return learning_units
...
jQuery.extend({
	...
	// A counter to track how many learning_units to wait for before
	...
	grep: function( elems, callback, inv ) {
		...
		// Go through the array, only saving the learning_units
	...
	map: function( elems, callback, arg ) {
		...
		// Go through the array, translating each of the learning_units to their
		if ( isArray ) {
			for ( ; i < length; i++ ) {
		...
		if ( (value && typeof value === "string") || value === undefined ) {
			...
			for ( i = 0, l = this.length; i < l; i++ ) {
				...
				if ( elem.nodeType === 1 && elem.className ) {
					...
					// loop over each learning_units in the removal list
		...
		if ( (match = rquickExpr.exec( selector )) ) {
			...
			if ( (m = match[1]) ) {
				if ( nodeType === 9 ) {
					...
					// nodes that are no longer in the document #6963
					if ( elem && elem.parentNode ) {
						// Handle the case where IE, Opera, and Webkit return learning_units
						// by name instead of ID
						if ( elem.id === m ) {
							results.push( elem );
							return results;
	...
	domManip: function( args, table, callback ) {
		...
		if ( this[0] ) {
			...
			if ( first ) {
				...
				// Use the original fragment for the last learning_units instead of the first because it can end up
...
function buildParams( prefix, obj, traditional, add ) {
	...
	if ( jQuery.isArray( obj ) ) {
		// Serialize array learning_units.
		jQuery.each( obj, function( i, v ) {
			if ( traditional || rbracket.test( prefix ) ) {
				// Treat each array learning_units as a scalar.
				add( prefix, v );

			} else {
				// If array learning_units is non-scalar (array or object), encode its
		...
		// Serialize object learning_units.
		for ( name in obj ) {
			buildParams( prefix + "[" + name + "]", obj[ name ], traditional, add );
		}

	} else {
		// Serialize scalar learning_units.
		add( prefix, obj );
	}
}
var
