...
class TasksController < ApplicationController
  ...
  def get_comments
    @comments = Comment.where(exercise_id: @task.id).order('created_at DESC')
  end
  ...
  def index
    ...
    when "like_counter"
      @tasks = Task.all.sort_by{|e| e.like_counter}
    when "like_counter DESC"
      @tasks = Task.all.sort_by{|e| e.like_counter}.reverse
    when "title"
      @tasks = Task.all.order('title')
    when "title DESC"
      @tasks = Task.all.order('title DESC')
    when "author_name"
      @tasks = Task.all.sort_by{|e| e.author_name.downcase}           # need to downcase to assure right order
    when "author_name DESC"
      @tasks = Task.all.sort_by{|e| e.author_name.downcase}.reverse
    else        # created_at DESC as default order
      puts "Undefined sort_by option"
      @tasks = Task.all.order('created_at DESC')
    end
    
    @top_task = @tasks.sort_by{|e| e.like_counter }.last    # important: find top_task before calling paginate!
  ...
  def show
    if @latest_performance != nil && @latest_performance.current_position > -1 then
      #last performance unfinished, set next exercises to do
      @next = Exercise.where(exercise_id: params[:id], position: @latest_performance.current_position).first
    end
    @comment = Comment.new      # needed for comment/_form.html.erb
    @existingLike = Like.where(exercise_id: params[:id], user_id: session[:current_user_id]).first #check, if user already likes this task
    @comments = @comments.paginate(:page => params[:page], :per_page => 10)
    @page = params[:page].nil? ? 1 : params[:page]
    @top_performance = Performance.where(exercise_id: @task.id, user_id: session[:current_user_id]).order('achieved_points DESC').first
  end

  def edit  # even admin isn't allowed to edit tasks, he/she may only delete it
    if @task.user_id != session[:current_user_id] then
  ...
  def create
    ...
    @task.save      # tasks created and saved
    ...
    tags.each do |t|
      ...
      if matching_tag != nil then
        ...
        to_create.exercise_id = @task.id
        to_create.content_id = matching_tag.id
        to_create.save   
      end
    end     # links between task and knowledge_elements created and saved
  ...
  def update
    ...
    old_tags = @task.knowledge_elements.map{|t| t.tag}                # tags as they were before editing
    ...
    tags_to_delete.each do |t|
      ...
      matching_tag = KnowledgeElement.find_by_sql("SELECT * FROM knowledge_elements WHERE tag = '" + t + "' COLLATE NOCASE;").first
      matching_link = matching_tag.nil? ? nil : ExerciseContent.where(content_id: matching_tag.id, exercise_id: @task.id).first
  ...
  def start
    if @task.subexercise_counter == 0 then
      redirect_to(exercise_path(@task), notice: "Keine Teilaufgaben vorhanden!")
    else
      firstQuestion = Exercise.where(exercise_id: @task.id).order('position ASC').first   # first question doens't have to have position 0
      ...
      @performance.max_points = @task.max_points
      @performance.save
      redirect_to perform_exercise_subexercise_path(@task, firstQuestion)
    end
  end
  ...
  def search
    ...
    if params[:search_style] == "tags" then
      ...
      ExerciseContent.all.each do  |l|
        if tags_to_search.include?(l.knowledge_elements.tag.downcase) then       # check for each existing link if its tag has eben entered
          @matching_tasks.push(l.task)                        # if yes, add corresponding tasks to matching_tasks
        end  
      end
      ...
      Task.all.each do |e|
        ...
        if e_title.match(input_title) || input_title.match(e_title) then    # check for each task if its title in
          @matching_tasks.push(e)
        end
      end
    else
       # TODO redirect to error page for other search_styles
    end
    @matching_tasks = @matching_tasks.paginate(:page => params[:page], :per_page => 10)
